...
- **Brain:** RP2040 reading the quadrature in hardware (PIO), presenting as USB HID. Firmware maps angular *velocity* to scrub speed via ramped J-K-L with a sub-threshold deadband, so the coast's exponential tail never drifts the playhead. Rate control, not position control — a free-coasting wheel can't couple position to the timeline.

Full design discussion (options, tradeoffs, the buy-vs-build analysis) is in `jog-wheel-session.md`; open tasks in `TODO.md`.
